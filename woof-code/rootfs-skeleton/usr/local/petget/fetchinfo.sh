...
case $DB_DISTRO in
 slackware)
  if [ ! -f /var/packages/PACKAGES.TXT-${DB_SUB} ];then
#  /usr/lib/gtkdialog/box_splash -font "8x16" -outline 0 -margin 4 -text "Please wait, downloading database file to /var/packages/PACKAGES.TXT-${DB_SUB}..." &
   if [ ! -f /tmp/install_quietly ]; then
    /usr/lib/gtkdialog/box_splash -close never -text "$(gettext 'Please wait, downloading database file to') /var/packages/PACKAGES.TXT-${DB_SUB}..." &
    X5PID=$!
   fi
   cd /var/packages
   case $DB_SUB in
 ...
 puppy|gentoo)
  ...
  REALHOME="`cat /var/packages/PKGS_HOMEPAGES | grep -i "$nPTN1" | head -n 1 | cut -f 2 -d ' '`"
  [ "$REALHOME" = "" ] && REALHOME="`cat /var/packages/PKGS_HOMEPAGES | grep -i "$nPTN2" | head -n 1 | cut -f 2 -d ' '`"
